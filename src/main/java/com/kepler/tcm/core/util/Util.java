...
public class Util {
	...
	public static void toMapValueString(Map propertyMap) {
		Iterator iterator = propertyMap.keySet().iterator();
		while(iterator.hasNext()){
			String strTemp = (String)iterator.next();
			propertyMap.put(strTemp, propertyMap.get(strTemp)+"");
		}
	}
	
	public static void main(String[] args) throws Exception {
		Map map = new HashMap();
		map.put("autoRestart","true");
		map.put("monitorInterval",""+300);
		map.put("serverPort",""+1099);
		map.put("monitorPort",1077);
		map.put("memo", "测试服务03");
		
		//toMapValueString(map);
		
		Iterator iterator = map.keySet().iterator();
		while (iterator.hasNext()) {
			String strTemp = (String) iterator.next();
			System.out.println(strTemp+":"+(String) map.get(strTemp));
		}
	}
}
