...
import com.kepler.tcm.server.tcm.server.Log4jManager;
...
public class StartMonitor {
	...
	public static void main(String[] args)
  {
    try
    {
      ...
      log4jManager.load();
      logger = Logger.getLogger("logger_monitor");

      StopMonitor sm = new StartMonitor().new StopMonitor();
      sm.setDaemon(true);
      sm.start();
      ...
      while (true)
      {
        Thread thread = new Thread(new StartMonitor().new Monitoring());
        thread.start();
	...
	class Restart extends Thread {
		...
		public void run() {
			try {
				Process p = Runtime.getRuntime().exec(StartMonitor.strExec);
