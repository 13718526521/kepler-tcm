...
public class MonitorThread implements Runnable {
	static final Logger logger = Logger.getLogger(MonitorThread.class);
	...
	public void run() {
		...
		try {
			monitorInterval = Config.getServerConfig(this.server, false).getInt("monitorInterval", -1);
		...
		logger.info("Begin to monitor " + Agent.host + ":" + this.monitorPort + "@" + this.server);
		while (true) {
			Socket socket = null;
			try {
				boolean live = false;
				try {
					...
					logger.error("请求" + Agent.host + ":" + this.monitorPort + "时异常.", e);
				}
