...
public class PluginManager {
		...
		try {
			...
			for (int i = 0; i < plugin.length; i++) {
				if (plugin[i].isDirectory()) {
					try {
						if ("classes;lib;plugins".indexOf(plugin[i].getName().toLowerCase()) < 0) {
							if (new File(plugin[i].getPath() + "_delete_plugin_flag").exists()) {
								FileTools.deleteDir(plugin[i].getPath(), true);
								if (!new File(plugin[i].getPath()).exists()) {
									new File(plugin[i].getPath() + "_delete_plugin_flag").delete();
								}
							} else {
								String id = plugin[i].getName();
								String profile = plugin[i].getPath() + "/" + "plugin.properties";
								if (!new File(profile).isFile()) {
									this.serverLogger.error("插件配置文件" + profile + "不存在，忽略此插件");
								} else {
									String fileList = FileTools.getFileText(plugin[i].getPath() + "/file.properties",
											Util.getCharset());

									PropertyResourceBundle prb = new PropertyResourceBundle(
											plugin[i].getPath() + "/" + "plugin.properties");
									if ((prb.getString("pluginName") == null)
											|| (prb.getString("pluginName").length() == 0)) {
										this.serverLogger.error("插件配置文件" + profile + "没有包含插件名称(pluginName=)，忽略此插件");
									} else {
										Plugin p = new Plugin(id, prb.getString("pluginName"),
												prb.getString("pluginMemo"), prb.getString("entryClass"),
												plugin[i].getPath(), fileList, "", System.currentTimeMillis());
						...
						this.serverLogger.error("加载插件'" + plugin[i].getPath() + "'时出现异常!", ex);
					}
				}
			}
		} catch (Exception e) {
	...
	public boolean addPlugin(HashMap propertyMap) throws Exception {
		try {
			...
			Util.writePropertyFile(tempMap, path + "/" + "plugin.properties");
			Plugin p = new Plugin(id, name, (String) propertyMap.get("pluginMemo"),
					(String) propertyMap.get("entryClass"), path, fileList, "", System.currentTimeMillis());
			this.alPluginIds.put(id, p);
	...
	public boolean modifyPlugin(HashMap propertyMap) throws Exception {
		try {
			...
			if (p != null) {
				...
				p = new Plugin(id, (String) propertyMap.get("pluginName"), (String) propertyMap.get("pluginMemo"),
						(String) propertyMap.get("entryClass"), path, fileList, "", modify);
	...
	public boolean removePlugin(String pluginId) throws Exception {
		try {
			...
			if (plugin != null) {
				new File(plugin.getPluginPath() + "_delete_plugin_flag").createNewFile();
	...
	public TaskListener getTaskInstance(String pluginId) {
		...
		result.setError("", new Exception("找不到插件：id=" + pluginId + ",将用ErrorClassHandleTask来接管此任务。请配置正确的任务插件并重启任务。"));
		return result;
	}
		...
		try {
			...
			if (plugin != null) {
				...
				if ((!f.exists()) || (f.delete())) {
					...
					Util.write(plugin.getPluginPath() + "/file.properties", false, text);
					plugin.setFileList(text);
				} else {
					throw new Exception("删除插件文件失败：" + file + " . " + f.getCanonicalPath());
				}
