...
public class ClassLoaderByClass extends ClassLoader {
	...
	protected Class findClass(String name) throws ClassNotFoundException {
		...
		try {
			Class localClass1;
			if (!this.isJar) {
				String className = FileTools.extractClassName(new ByteArrayInputStream(this.classBytes));
				c = defineClass(className.replace('/', '.'), this.classBytes, 0, this.classBytes.length);
				if (classOf(c, this.base))
					return c;

			} else {
				jar = new JarInputStream(new ByteArrayInputStream(this.classBytes));
				JarEntry entry;
				while ((entry = jar.getNextJarEntry()) != null) {
					...
					if (!ename.endsWith(".class")) {
						...
						jar.closeEntry();
						c = defineClass(ename, baos.toByteArray(), 0, baos.size());
						baos.close();
	...
	public static void main(String[] args) throws Exception {
		ClassLoaderByClass cl = new ClassLoaderByClass(
				"D:\\workspaces\\workspace_dbg\\web_tcm_plugin\\classes\\tcm\\plugin\\demo1\\Demo.class", Task.class);
		...
		ClassLoaderByClass cl1 = new ClassLoaderByClass("c:/people_xinhua.jar", Task.class);
